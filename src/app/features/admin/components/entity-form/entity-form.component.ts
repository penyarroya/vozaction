...
export class EntityFormComponent implements OnInit, AfterViewInit {
  ...
  // Inputs para soportar tanto modo ruta como modo componente/modal directo
  entityInput = input<string | null>(null, { alias: 'entity' });
  dataInput = input<any>(null, { alias: 'data' });
  isEditingInput = input<boolean | null>(null, { alias: 'isEditing' });

  // Inputs tradicionales (por compatibilidad)
  loading = input(false);
  onSave = output<any>();
  onCancel = output<void>();
  ...
  cancelar() {
    console.log('❌ Cancelando formulario');
    // Si viene por ruta, navega hacia atrás; de lo contrario, solo emite el evento de cancelar
    if (!this.entityInput()) {
      this.router.navigate(['/admin/entities', this.entityName()]);
    }
    this.onCancel.emit();
